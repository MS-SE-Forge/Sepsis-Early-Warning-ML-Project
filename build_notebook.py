...
from the artifacts saved by build_artifacts.py.

Run this AFTER build_artifacts.py has completed successfully and
...
> **Note for the team:** Fill in the four fields above before running `build_notebook.py`.
...
code("""# Load official scorer output.
# Generated by running: python utility_eval_prep.py
# followed by the PhysioNet evaluation script.
# Falls back to test predictions if the file is not present.
import os

if os.path.exists("utility_results.psv"):
    ...
    # Fallback: recompute AUROC/AUPRC from saved test predictions
    test_preds = pd.read_csv("artifacts/test_predictions.csv")
    from sklearn.metrics import roc_auc_score, average_precision_score
    auroc_val = roc_auc_score(test_preds['label'], test_preds['pred_prob'])
    auprc_val = average_precision_score(test_preds['label'], test_preds['pred_prob'])
    acc_val   = None
    util_val  = None
    print("utility_results.psv not found — showing metrics from saved test predictions.")
...
with open("artifacts/error_examples.json") as f:
...
A demonstration using saved artifacts. Full pipeline training on the complete 40,336-patient dataset takes several minutes on commodity hardware; we demonstrate real model inference on a small but representative sample of held-out test patients, while retaining the ability to explain and fully reproduce the training process.
""")
